...
- For motion follow-ups on state-transition slides, keep the conceptual `segments` and `notes` but add compact `transaction`, `stateBefore`, `stateAfter`, and `executionSteps` fields so the animation can show one explicit before/after snapshot without duplicating the lesson copy
- Smart-contract lifecycle motion slides can also reuse the shared `evm-state-motion` layout: keep the deploy/call/execute/update teaching arc in `segments`, then drive the animation with one concrete contract call plus `stateBefore` and `stateAfter` snapshots instead of building a separate contract-only renderer
- DeFi motion follow-ups can reuse the base primitive slide by swapping to the shared `defi-journey-motion` layout and adding `journeyLabel`, `journeySteps`, `walletStart`, and `walletFinish`; keep the existing `segments` and `notes` so the static teaching model and animated user flow stay synchronized
- DeFi fragility motion slides can reuse the shared `defi-cascade-motion` layout: feed it `stages`, `healthySnapshot`, `stressSnapshot`, and `incidentSteps` so the slide teaches the useful composed path first, then shows one dependency shock rippling through the stack
