...
- Consensus comparison slides can also reuse the shared `ledger` layout: use one card for Proof of Work, one for Proof of Stake, and the side flow to explain how each model turns a candidate block into accepted history
...
- Smart-contract process-flow diagram slides can also reuse the shared `pipeline` layout: make each stage one visible handoff such as wallet, contract call, EVM execution, and state update so the diagram reads left to right without bespoke React
- Protocol-upgrade and ecosystem-coordination slides can also reuse the shared `pipeline` layout: map the stages to public discussion, EIP/research, client implementation, and network adoption so governance explainers stay sequence-driven without bespoke React code
...
- Minimal definition slides can also reuse the shared `anatomy` layout: keep them to three short segments such as what it is, what it does, and why it matters so concept introductions stay bullet-led and easy to scan
- Participation-choice slides can also reuse the shared `anatomy` layout: compare the self-run path, the provider path, and the reward/tradeoff summary so staking or custody choices stay simple without building custom React
...
- Principle-to-manifestation slides can also reuse the shared `anatomy` layout: make each segment pair one value with a visible ecosystem choice so culture and governance slides stay concrete instead of turning into slogan lists
- Category-tour slides can also reuse the shared `anatomy` layout: make each segment one protocol primitive or function so learners can scan what swaps, lending, stablecoins, or similar building blocks each do without adding bespoke React code
- Ecosystem role-mapping slides can also reuse the shared `anatomy` layout: give each segment one actor group, its responsibility, and how it relates to the rest of the network so community chapters stay concrete without implying central control
- Resource-pricing slides can also reuse the shared `anatomy` layout: split the concept into what the protocol meters, why that pricing exists, and what misconception to avoid so gas or fee slides stay principled without drifting into formula details too early
- EOA and key-management slides can also reuse the shared `anatomy` layout: separate the public account, the recovery secret, and the signing act so beginner wallet explanations stay precise without implying the seed phrase is used directly on-chain
- Wallet transaction-flow slides can also reuse the shared `anatomy` layout: map the interface, transaction builder, signing path, signature verification, and RPC broadcast into segments so learners can distinguish wallet UX from the node-facing network flow without custom React
- Multi-model tradeoff slides can also reuse the shared `anatomy` layout: make each segment one operating model and use the notes to spell out who controls risk, recovery, and UX rather than forcing a three-way comparison into the two-card `ledger` template
- Security-assumption tradeoff slides can also reuse the shared `anatomy` layout: make each segment one comparison dimension such as attack cost, honest participation, recovery, or attack surface, then compare Proof of Work and Proof of Stake inside each card instead of squeezing the topic into a simple two-column verdict
- User-security slides can also reuse the shared `anatomy` layout: make each segment one attack surface cluster such as phishing, dangerous approvals, or compromised environments so OPSEC chapters stay practical without custom React work
- Practical checklist or habit slides can also reuse the shared `anatomy` layout: make each segment one repeatable behavior cluster so closing guidance stays actionable instead of turning into a dense bullet list
- Composability-versus-fragility slides can reuse the shared `ledger` layout: use one card for the stacked protocol upside, the other for cascading failure modes and practical limits, then use the side flow to show how risk moves across the dependency chain
- Fee-market slides can also reuse the shared `ledger` layout: contrast calm versus crowded block space in the two cards, then use the side flow to map wallet quote, pending competition, and inclusion priority into the user-facing wait/cost tradeoff
...
- For shared `ledger` slides that need the intro row separated from the comparison cards, prefer slide-specific CSS on the slide-number class and use grid `align-content: space-between` on the layout instead of forking the shared React renderer
- If a `ledger` slide needs the extra space to live above the comparison row rather than inside the cards, add slide-specific `margin-top` on `.comparison-grid` and reduce that slide's `.comparison-card` `min-height` instead of stretching the whole layout
- The default shared `ledger` comparison treatment can carry a top gap above `.comparison-grid` plus a shorter `.comparison-card` min-height, because many explainer slides read better with the whitespace between intro and cards rather than trapped inside the cards
...
- When a slide's core idea is a sequence, state transition, or cascade, add a separate Remotion-focused story to `prd.json` instead of burying the animation work inside the base slide story notes
...
- For motion follow-ups to an existing explainer slide, keep the same structured slide data in `src/data/slides.js` when possible and swap only the `layout`; that keeps the static teaching model and the animated version aligned instead of duplicating content in two places
- For motion follow-ups on process-plus-schedule slides, keep the existing `process` and `eras` fields and let the animated layout consume those directly; this avoids inventing a second data shape for the same lesson
- Consensus comparison motion slides can reuse the shared `consensus-motion` layout: define paired lane objects with `resourceValue`, `candidate`, `signal`, `accepted`, and `steps` fields so Proof of Work and Proof of Stake stay visually parallel without building slide-specific React
...
- Speaker or facilitator intro slides can live as their own top-level group at slide `0` with a dedicated `speaker-intro` layout; store the portrait path, role line, bio, and social handle in `src/data/slides.js`, keep the renderer generic in `src/App.jsx`, and point portraits at files in `public/`
