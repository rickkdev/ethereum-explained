...
- For structural explainer slides that need to break one concept into named parts, use the shared `anatomy` layout in `src/App.jsx` with `frameLabel`, `segments`, and `notes` in `src/data/slides.js` instead of forcing the content into the `ledger` comparison template
